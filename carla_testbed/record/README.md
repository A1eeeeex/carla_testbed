...
- `autoware_operator_view.py`：Autoware demo 的 RViz2 operator-view 启动与 X11/ffmpeg 录屏。
- `autoware_rosbag.py`：Autoware 容器内 rosbag2 topic 归档。
...
- `runs/<run>/video/rviz/autoware_rviz.mp4`（Autoware operator-view demo，可选）
- `runs/<run>/rosbag2/autoware_demo/`（Autoware rosbag2 demo，可选）
...
- Autoware RViz 视频缺失：检查 `DISPLAY`、Docker、Autoware 镜像内 `rviz2`，查看 `artifacts/autoware_operator_view_status.json`。
- Autoware rosbag 缺失：检查容器是否仍在运行、topic 是否存在，查看 `artifacts/autoware_rosbag_status.json`。
