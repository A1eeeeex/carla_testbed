...
class SimpleAccRouteFollowerController(Controller):
    ...
    def step(self, frame: FrameStamp, ego: EgoState, scene: SceneTruth) -> ControlCommand:
        ...
        lead = _nearest_lead(ego, scene, lateral_gate_m=self.config.lead_lateral_gate_m)
        desired_gap = None
...
def _nearest_lead(ego: EgoState, scene: SceneTruth, *, lateral_gate_m: float) -> LeadObservation | None:
    ...
    for obstacle in scene.obstacles:
        ...
        if gap <= 0.0 or abs(lateral) > lateral_gate_m:
            continue
        speed = math.sqrt(
...
def _steer_from_route_errors(ego_metadata: dict[str, Any], scene_metadata: dict[str, Any], cfg: SimpleAccRouteFollowerConfig) -> float:
    heading_error = _metadata_float(ego_metadata, scene_metadata, "heading_error_rad")
