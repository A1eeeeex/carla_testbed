# 模块定位
提供测试台的配置解析与派生逻辑：加载 rig 预设/自定义文件，展开为传感器规格，生成校准(calibration)、时间同步(time_sync)、噪声(noise_model)、数据格式(data_format)与 I/O 契约(io_contract_ros2/ cyber)。不负责仿真运行或文件写入之外的业务。

# 目录与关键文件
- `defaults.py`：基础默认参数与 HarnessConfig 定义。
- `rig_loader.py`：加载 rig 预设/文件，支持 `--rig`/`--rig-file`/`--rig-override`，并生成传感器规格。
- `rig_postprocess.py`：根据 rig 生成 meta、calibration、time_sync、noise_model、data_format、io_contract_*；包含保存工具。
- `__init__.py`：导出 HarnessConfig 及默认配置。

# 对外接口（Public API）
- `HarnessConfig`（defaults.py）：运行入口的总配置 dataclass（host/port/ticks/record 等）。
...
- `expand_specs(meta_typed, rig_final, rig_name)`（rig_postprocess.py）：生成展开的 sensors_expanded 结构。
- `derive_calibration/derive_time_sync/derive_noise/derive_data_format/derive_io(expanded, backend)`：派生各类配置文件数据。
- `save_json(path, obj)`：保存 JSON/YAML 兼容内容。

# 数据契约（I/O Contract）
- 输入：rig 配置（内置或自定义），可通过 CLI `--rig*` 传入。
- 输出（写入 `runs/<run>/config/`）：
  - `sensors_expanded.json`（展开后的传感器规格与 frame_id）
  ...
  - `io_contract_ros2.yaml` / `io_contract_cyber.yaml`（按传感器生成 topic/msg/QoS 契约）
...
- 外参/TF 错位：确认 rig transform 单位为米/度，坐标系为 CARLA world（x 前 y 右 z 上）。
- io_contract 缺少某传感器：确保 rig 中 `enabled: true`，且 `type` 支持的后端。
- `KeyError` 在 overrides：检查 key 路径是否存在，点路径需匹配结构（如 camera_front.attributes.fov）。
- 时间同步字段为空：确认 `derive_time_sync` 调用后文件写入成功（runs/<run>/config/time_sync.json）。

# 与其他模块的关系
- 上游：CLI/场景选择 rig；无外部依赖。
- 下游：sensors 模块读取 specs；runner 在 run 开始时写出 calibration/time_sync/io_contract；io/record 使用这些配置。
- 调用路径：examples/run_followstop.py -> rig_loader/rig_postprocess -> runs/<run>/config。

# Roadmap
- 支持自动校验自定义 rig 的合法性（类型/属性约束）。
- 扩展 contract 生成器以适配更多中间件。 
