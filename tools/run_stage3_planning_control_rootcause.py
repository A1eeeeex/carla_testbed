...
def _run_case(case: Dict[str, Any], run_dir: Path, overrides: List[str]) -> None:
    ...
    print(f"[stage3] running {case['label']} -> {run_dir}")
...
def _run_analyzer(batch_root: Path) -> None:
    cmd = CONDA_PREFIX + [
        "python",
        str(REPO_ROOT / "tools" / "analyze_stage3_planning_control_rootcause.py"),
        "--batch-root",
        str(batch_root),
    ]
    subprocess.run(cmd, check=True, cwd=str(REPO_ROOT), env=os.environ.copy())
...
def main() -> None:
    ...
    (batch_artifacts / "stage3_cases.json").write_text(json.dumps(selected_cases_payload, indent=2))
    (batch_artifacts / "stage3_commands.txt").write_text("\n".join(commands) + "\n")

    if args.skip_run:
        print(f"[stage3] commands written to {batch_artifacts / 'stage3_commands.txt'}")
        return
