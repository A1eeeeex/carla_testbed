...
CASES: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "config": REPO_ROOT / "configs" / "io" / "examples" / "followstop_apollo_gt_case3_tightened_locked.yaml",
    },
]
...
def _run_case(case: Dict[str, Any], run_dir: Path, overrides: List[str]) -> None:
    ...
    print(f"[stage5] running {case['label']} -> {run_dir}")
...
def _run_analyzer(batch_root: Path) -> None:
    cmd = CONDA_PREFIX + [
        "python",
        str(REPO_ROOT / "tools" / "analyze_stage5_mapdir_reroute_lanefollow.py"),
        "--batch-root",
        str(batch_root),
    ]
    subprocess.run(cmd, check=True, cwd=str(REPO_ROOT), env=os.environ.copy())
...
def main() -> None:
    ...
    parser.add_argument("--cases", default="A,B,BFREEZE,BTIGHTENED")
    ...
    (batch_artifacts / "stage5_cases.json").write_text(json.dumps(payload, indent=2))
    commands: List[str] = []
    ...
    (batch_artifacts / "stage5_commands.txt").write_text("\n".join(commands) + "\n")

    if args.skip_run:
        print(f"[stage5] commands written to {batch_artifacts / 'stage5_commands.txt'}")
        return
