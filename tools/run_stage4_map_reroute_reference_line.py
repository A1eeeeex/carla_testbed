...
def _run_case(case: Dict[str, Any], run_dir: Path, overrides: List[str]) -> None:
    ...
    print(f"[stage4] running {case['label']} -> {run_dir}")
...
def _run_analyzer(batch_root: Path, baseline_batch_root: str) -> None:
    cmd = CONDA_PREFIX + [
        "python",
        str(REPO_ROOT / "tools" / "analyze_stage4_map_reroute_reference_line.py"),
        "--batch-root",
        str(batch_root),
    ]
...
def main() -> None:
    ...
    (batch_artifacts / "stage4_cases.json").write_text(json.dumps(payload, indent=2))
    commands: List[str] = []
    ...
    (batch_artifacts / "stage4_commands.txt").write_text("\n".join(commands) + "\n")

    if args.skip_run:
        print(f"[stage4] commands written to {batch_artifacts / 'stage4_commands.txt'}")
        return
