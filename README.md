...
若需 demo 录制与 HUD overlay（依赖 pillow + ffmpeg）：
```bash
python carla_testbed/examples/run_followstop.py --record-demo --make-hud
```
...
8) **视频/HUD/传感器录制缺失**：新框架未移植旧的 demo 录制；暂用 `code/followstop/test_followstop_policy.py --record-demo` 跑，或参考 `step1_record_demo.py` 迁移。
