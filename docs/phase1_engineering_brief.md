...
| scenario case catalog | Enumerate reusable ScenarioCases | DONE | `carla_testbed/analysis/phase1_scenario_catalog.py`; `tools/phase1_scenario_catalog.py`; `artifacts/phase1_scenario_catalog_current/phase1_scenario_catalog.json`; `configs/scenarios/`; `configs/scenario_templates/`; `tests/test_phase1_scenario_catalog.py` | Catalog summary records YAML, template compile or route-only no-target applicability, concrete `scenario_case_ids`, target actor contract, CARLA online, Apollo online, Apollo fixed-scene readiness, Apollo fixed-scene dispatch-contract evidence, Apollo fixed-scene runtime-dispatch status, `v-t-gap`, comparison evidence, safety-event evidence-surface consistency, and explicit evidence-root ingestion separately. Latest local catalog reports all 8 current P0/P1 rows DONE. DONE means scenario-platform evidence and comparable surfaces exist; several representative backend runs still fail behaviorally. | Keep the catalog report in review packs and keep new scenarios from being marked DONE without online ApolloBackend plus PlanningControlBackend evidence. |
...
| cut_out_simple | P1 target exits ego lane | DONE | `configs/scenario_templates/cut_out.yaml`; `configs/scenarios/town01/cut_out_097.yaml`; `configs/scenarios/baguang/cut_out_35kph_right_to_left_25m.yaml`; `runs/phase1_apollo_sidecar_cut_out_overlay_online_20260620_010728/analysis/phase1_status/phase1_status.json`; `runs/phase1_builtin_baguang_cut_out_safety_surface_20260620_010655/analysis/phase1_status/phase1_status.json`; `runs/phase1_comparisons/baguang_cut_out_apollo_overlay_vs_builtin_safety_surface_20260620_010954/comparison_summary.json`; `carla_testbed/scenario_player/templates/cut_out.py`; `tests/test_fixed_scene_compiler.py::test_baguang_cut_out_compiles_lane_change_phase`; `tests/test_apollo_fixed_scene_launch_plan.py::test_apollo_baguang_cut_out_launch_plan_uses_sidecar_runtime_command` | DONE means cut-out now has a Baguang fixed-scene ScenarioCase plus same-case ApolloBackend-vs-PlanningControlBackend comparison evidence. It does not mean behavior success: both representative runs are evaluable `failed/lane_invasion`. The Baguang cut-out variant uses a fixed 35s diagnostic duration so lane-change playback can be validated without relying on road-end progression. | Keep Town01 cut-out as future map-specific coverage; use the Baguang comparable failure to triage early lane invasion and Apollo module-consumption blockers. |
...
| lead_hard_brake | P1 hard-brake following | DONE | `configs/scenarios/baguang/lead_hard_brake_70_to_0_20m.yaml`; `runs/phase1_apollo_sidecar_lead_hard_brake_overlay_online_20260620_004446/analysis/phase1_status/phase1_status.json`; `runs/phase1_apollo_sidecar_lead_hard_brake_overlay_online_20260620_004446/analysis/phase1_apollo_fixed_scene_dispatch/phase1_apollo_fixed_scene_dispatch_report.json`; `runs/phase1_builtin_baguang_lead_hard_brake_safety_surface_20260620_004737/analysis/phase1_status/phase1_status.json`; `runs/phase1_comparisons/baguang_lead_hard_brake_apollo_overlay_vs_builtin_safety_surface_20260620_004757/comparison_summary.json`; `tests/test_fixed_scene_compiler.py::test_baguang_lead_hard_brake_profile_matches_70_to_stop` | DONE means the hard-brake ScenarioCase now has same-case ApolloBackend-vs-PlanningControlBackend comparison evidence with matched safety-event counter surfaces. It does not mean behavior success: both representative runs are evaluable but failed with `lane_invasion`; Apollo link-health reports `apollo_module_consumption:routing_response_not_consumed_by_planning` as the current primary blocker. This is Phase 1 comparison evidence only, not Apollo hard-brake capability evidence. | Keep this comparison current and use it as a target-following blocker-reduction case. |
...
- The generated catalog currently reports `8` DONE scenarios, `0` PARTIAL, and
  `0` NOT_YET. DONE means Phase 1 scenario-platform evidence is present; it
  does not mean Apollo natural-driving success. The hard-brake Apollo sidecar
...
- `cut_out_simple` now has a Baguang fixed-scene ScenarioCase,
  ...
  behavior pass.
- The current catalog reports `8` DONE scenarios, `0` PARTIAL, and `0`
  `NOT_YET`.
