...
Latest reviewed snapshot: `2026-06-20`, after the accepted-bundle gate was
applied to the current run evidence.
...
- All current P0/P1 rows have scenario definitions and comparable
  ApolloBackend-vs-PlanningControlBackend surfaces.
- `lane_keep_straight`, `follow_stop_static`, `lead_decel_accel`,
  `lane_keep_curve`, `cut_in_simple`, `cut_out_simple`,
  `junction_turn_no_signal`, and `lead_hard_brake` now have accepted
  `phase1_acceptance_report.json` bundles binding one exact ApolloBackend run,
  one exact PlanningControlBackend run, and one exact ScenarioComparison with
  all Phase 1 validity gates true.
- The next phase-progress milestone is no longer catalog coverage. It is
  reducing the representative Apollo blockers inside accepted comparison
  surfaces, starting with Baguang/Town01 lane-event failures and the current
  `prediction_evidence:closed_loop` link-health blocker. The latest refreshed
  cut-in evidence makes that blocker more precise: Apollo Prediction runtime
  was observed, but no `/apollo/prediction` channel output was observed, so
  the dynamic cut-in claim remains blocked rather than silently bypassed with
  GT obstacles.
- Existing comparable failures remain useful blocker evidence. They must not be
  rewritten as Apollo natural-driving success; Phase 1 completion here means
  accepted comparison-surface coverage, not backend behavior success.
...
| scenario case catalog | Enumerate reusable ScenarioCases | DONE | `carla_testbed/analysis/phase1_scenario_catalog.py`; `tools/phase1_scenario_catalog.py`; `carla_testbed/analysis/phase1_acceptance.py`; `tools/phase1_acceptance.py`; `configs/scenarios/`; `configs/scenario_templates/`; `runs/phase1_acceptance_cut_in_simple_20260620_170727/acceptance/phase1_acceptance_report.json`; `tests/test_phase1_scenario_catalog.py`; `tests/test_phase1_acceptance.py` | Catalog summary records YAML, template compile or route-only no-target applicability, concrete `scenario_case_ids`, target actor contract, CARLA online, Apollo online, Apollo fixed-scene readiness, Apollo fixed-scene dispatch-contract evidence, Apollo fixed-scene runtime-dispatch status, `v-t-gap`, comparison evidence, safety-event evidence-surface consistency, accepted bundle status, and explicit evidence-root ingestion separately. The previous 8/8 DONE reading was too optimistic because it mixed best available evidence across runs; the current 8/8 DONE reading is backed by accepted `phase1_acceptance_report.json` bundles for every current P0/P1 row. | Keep the catalog report in review packs and keep future scenarios from being marked DONE without an accepted Phase1AcceptanceBundle. |
| follow_stop_static | P0 static lead stop | DONE | `configs/scenarios/baguang/follow_stop_static_300m.yaml`; `configs/scenarios/baguang/follow_stop_static_300m_spawn2m.yaml`; `configs/scenarios/town01/follow_stop_097.yaml`; `configs/scenario_templates/static_lead_stop.yaml`; `configs/io/examples/phase1_baguang_apollo_followstop_static_spawn2m_compat.yaml`; `runs/phase1_static_followstop_auto_projection_online2_20260619_173104/analysis/phase1_status/phase1_status.json`; `runs/phase1_static_followstop_auto_projection_online2_20260619_173104/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_online_builtin_follow_stop_static_spawn2m_20260619_165713/analysis/phase1_status/phase1_status.json`; `runs/phase1_online_builtin_follow_stop_static_spawn2m_20260619_165713/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_acceptance_follow_stop_static_20260620_154807/comparison/comparison_summary.json`; `runs/phase1_acceptance_follow_stop_static_20260620_154807/acceptance/phase1_acceptance_report.json`; `tests/test_fixed_scene_compiler.py`; `tests/test_phase1_scenario_catalog.py`; `tests/test_baguang_lane_event_contract.py`; `tests/test_scenario_comparison.py`; `tests/test_apollo_fixed_scene_launch_plan.py`; `tests/test_phase1_acceptance.py`; `tests/test_phase1_run_postprocess.py` | The accepted bundle proves the exact ApolloBackend run, PlanningControlBackend run, comparison, resolved `lead_vehicle` target, `v_t_gap` evidence, Phase 1 artifact completeness, and no-blocking-assist gates are all bound atomically for this scenario. The Apollo run remains an evaluable `lane_invasion` / downstream blocker failure, so this is accepted Phase 1 comparison-surface evidence, not Apollo follow-stop success or natural-driving capability evidence. | Keep this accepted bundle as the target-actor reference pattern and apply the same atomic gate to the remaining `cut_in_simple` blocker. |
| lead_decel_accel | P0 lead speed-change following | DONE | `configs/scenario_templates/lead_vehicle_accel_decel.yaml`; `configs/scenarios/town01/lead_accel_decel_097.yaml`; `configs/scenarios/baguang/lead_accel_40_to_70_20m.yaml`; `configs/scenarios/baguang/lead_decel_70_to_40_20m.yaml`; `runs/phase1_apollo_sidecar_lead_decel_overlay_online_20260619_202831/analysis/phase1_status/phase1_status.json`; `runs/phase1_apollo_sidecar_lead_decel_overlay_online_20260619_202831/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_builtin_baguang_lead_decel_safety_surface_20260619_210418/analysis/phase1_status/phase1_status.json`; `runs/phase1_builtin_baguang_lead_decel_safety_surface_20260619_210418/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_acceptance_lead_decel_accel_20260620_155835/comparison/comparison_summary.json`; `runs/phase1_acceptance_lead_decel_accel_20260620_155835/acceptance/phase1_acceptance_report.json`; `tests/test_fixed_scene_compiler.py`; `tests/test_v_t_gap_extractor.py`; `tests/test_phase1_run_postprocess.py`; `tests/test_scenario_comparison.py`; `tests/test_phase1_acceptance.py` | The accepted bundle binds exact ApolloBackend and PlanningControlBackend runs for the Baguang lead-deceleration case, resolved `lead_vehicle` target evidence, `v_t_gap`, Phase 1 artifact completeness, and no-blocking-assist gates. Both participating runs are still evaluable `lane_invasion` failures, so this is accepted Phase 1 comparison-surface evidence only, not Apollo natural-driving or lead-following success. | Use this accepted bundle for blocker reduction and keep separate online work focused on reducing lane-event and Apollo downstream blockers. |
| cut_in_simple | P0 target enters ego lane | DONE | `configs/scenario_templates/cut_in.yaml`; `configs/scenarios/baguang/cut_in_35kph_left_to_right_10m.yaml`; `configs/scenarios/town01/cut_in_097.yaml`; `runs/phase1_apollo_sidecar_cut_in_target_evaluable_online_20260620_165920/analysis/phase1_status/phase1_status.json`; `runs/phase1_apollo_sidecar_cut_in_target_evaluable_online_20260620_165920/analysis/prediction_evidence/prediction_evidence_report.json`; `runs/phase1_apollo_sidecar_cut_in_target_evaluable_online_20260620_165920/analysis/apollo_link_health/apollo_link_health_report.json`; `runs/phase1_builtin_baguang_cut_in_target_evaluable_20260620_170548/analysis/phase1_status/phase1_status.json`; `runs/phase1_comparisons/baguang_cut_in_target_evaluable_refreshed_20260620_170727/comparison_summary.json`; `runs/phase1_acceptance_cut_in_simple_20260620_170727/acceptance/phase1_acceptance_report.json`; `tests/test_fixed_scene_player.py::test_baguang_cut_in_starts_at_configured_activation_gap`; `tests/test_scenario_actor_contract.py::test_lane_change_contract_uses_actor_world_motion_when_ego_relative_lateral_drifts`; `tests/test_phase1_status_classifier.py::test_duration_policy_not_reached_after_target_interaction_is_evaluable_failure`; `tests/test_apollo_prediction_evidence.py::test_dynamic_case_with_prediction_runtime_but_no_output_fails_explicitly`; `tests/test_apollo_link_health.py::test_dynamic_prediction_placeholder_is_regenerated_and_written_before_consumption` | The accepted bundle binds exact ApolloBackend and PlanningControlBackend runs for the Baguang cut-in target-transition case. The scenario now starts inside the target activation window so both backends exercise `cut_in_lane_change`; actor-world motion is used for no-teleport evidence when ego-relative lateral is contaminated by ego lane departure. The refreshed Apollo evidence shows Prediction runtime was observed but `/apollo/prediction` output was missing, so dynamic cut-in remains blocked by `prediction_evidence:closed_loop`. Both participating runs remain evaluable `lane_invasion` failures, so this is accepted Phase 1 comparison-surface evidence only, not Apollo cut-in behavior success. | Use this accepted bundle as target-transition blocker evidence; next work should reduce Apollo/Baguang lane-event and native prediction-output blockers without weakening no-interference or natural-driving claim gates. |
| lane_keep_straight | P0 lateral baseline | DONE | `configs/scenarios/town01/lane_keep_097.yaml`; `runs/phase1_online_builtin_lane_keep_safety_surface_20260619_163934/`; `runs/apollo_l1_accel_filter_tuned_20260616_115254/.../analysis/phase1_status/phase1_status.json`; `runs/phase1_acceptance_lane_keep_straight_20260620_153947/comparison/comparison_summary.json`; `runs/phase1_acceptance_lane_keep_straight_20260620_153947/acceptance/phase1_acceptance_report.json`; `tests/test_phase1_scenario_catalog.py`; `tests/test_scenario_comparison.py`; `tests/test_phase1_acceptance.py`; `tests/test_run_artifact_completeness.py` | The accepted bundle binds the exact ApolloBackend run, PlanningControlBackend run, ScenarioComparison, Phase 1 artifact-completeness reports, route-only target metric `not_applicable`, and no-blocking-assist gates. Apollo's participating run is still an evaluable `lane_invasion` failure, so this is accepted Phase 1 comparison-surface evidence, not Apollo natural-driving pass evidence. Any natural-driving claim would still require `natural_driving_report.json` plus the no-interference artifacts. | Keep lane-keep comparison evidence current and focus follow-up on reducing the representative Apollo blocker while closing the remaining `cut_in_simple` acceptance blocker. |
| lane_keep_curve | P0 lateral curve baseline | DONE | `configs/scenarios/town01/curve217_diagnostic.yaml`; `runs/apollo_cadence_hotpath_validation2_20260617_091626/curve_lane_follow__adhoc__town01_rh_spawn068_goal079__seed/curve_lane_follow__adhoc__town01_rh_spawn068_goal079__seed__manual_online_chain__town01_rh_spawn068_goal079/analysis/phase1_status/phase1_status.json`; `runs/apollo_cadence_hotpath_validation2_20260617_091626/curve_lane_follow__adhoc__town01_rh_spawn068_goal079__seed/curve_lane_follow__adhoc__town01_rh_spawn068_goal079__seed__manual_online_chain__town01_rh_spawn068_goal079/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_online_builtin_curve217_diagnostic_20260617_135025/analysis/phase1_status/phase1_status.json`; `runs/phase1_online_builtin_curve217_diagnostic_20260617_135025/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_acceptance_lane_keep_curve_20260620_155918/comparison/comparison_summary.json`; `runs/phase1_acceptance_lane_keep_curve_20260620_155918/acceptance/phase1_acceptance_report.json`; `tests/test_phase1_status_classifier.py`; `tests/test_scenario_comparison.py`; `tests/test_phase1_acceptance.py`; `tests/test_run_artifact_completeness.py` | The accepted bundle binds exact ApolloBackend and PlanningControlBackend route-only curve runs with target metric `not_applicable`, Phase 1 artifact completeness, and no-blocking-assist gates. Apollo remains evaluable but failed with `route_establishment_latency`; the representative blocker remains reference-line/projection/runtime evidence, so this is not Apollo curve-driving success. | Use the accepted comparison surface to guide Apollo reference-line/projection follow-up; do not tune control from this result alone. |
| cut_out_simple | P1 target exits ego lane | DONE | `configs/scenario_templates/cut_out.yaml`; `configs/scenarios/town01/cut_out_097.yaml`; `configs/scenarios/baguang/cut_out_35kph_right_to_left_25m.yaml`; `runs/phase1_acceptance_cut_out_simple_20260620_1622/runs/phase1_apollo_sidecar_cut_out_overlay_online_20260620_010728/analysis/phase1_status/phase1_status.json`; `runs/phase1_acceptance_cut_out_simple_20260620_1622/runs/phase1_builtin_baguang_cut_out_safety_surface_20260620_010655/analysis/phase1_status/phase1_status.json`; `runs/phase1_acceptance_cut_out_simple_20260620_1622/comparison/comparison_summary.json`; `runs/phase1_acceptance_cut_out_simple_20260620_1622/acceptance/phase1_acceptance_report.json`; `carla_testbed/scenario_player/templates/cut_out.py`; `tests/test_fixed_scene_compiler.py::test_baguang_cut_out_compiles_lane_change_phase`; `tests/test_fixed_scene_contract.py::test_lane_change_progress_can_complete_phase_without_stop_event`; `tests/test_phase1_acceptance.py`; `tests/test_apollo_fixed_scene_launch_plan.py::test_apollo_baguang_cut_out_launch_plan_uses_sidecar_runtime_command` | The accepted bundle binds exact ApolloBackend and PlanningControlBackend runs for the Baguang cut-out target-transition case. The fixed-scene contract now accepts explicit lane-change trace completion evidence (`lane_change_progress >= 0.95`) when the ego run ends before the scene stop event. Both participating runs remain evaluable `lane_invasion` failures, so this is accepted Phase 1 comparison-surface evidence only, not Apollo cut-out or natural-driving capability evidence. | Use this accepted bundle as target-transition blocker evidence; keep Town01 cut-out as future map-specific coverage and do not treat the lane-change playback evidence as ego autonomy success. |
| junction_turn_no_signal | P1 junction/turn case | DONE | `configs/scenarios/town01/junction_031.yaml`; `runs/phase1_apollo_junction031_online_lateral_20260619_181629/junction_traverse__town01_rh_spawn031_goal056/analysis/phase1_status/phase1_status.json`; `runs/phase1_apollo_junction031_online_lateral_20260619_181629/junction_traverse__town01_rh_spawn031_goal056/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_online_builtin_junction_031_20260617_135035/analysis/phase1_status/phase1_status.json`; `runs/phase1_online_builtin_junction_031_20260617_135035/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_acceptance_junction_turn_no_signal_20260620_155921/comparison/comparison_summary.json`; `runs/phase1_acceptance_junction_turn_no_signal_20260620_155921/acceptance/phase1_acceptance_report.json`; `tests/test_phase1_scenario_binding.py`; `tests/test_scenario_comparison.py`; `tests/test_phase1_acceptance.py`; `tests/test_run_artifact_completeness.py` | The accepted bundle binds exact ApolloBackend and PlanningControlBackend route-only junction runs with target metric `not_applicable`, Phase 1 artifact completeness, and no-blocking-assist gates. The Apollo run remains evaluable but failed with `route_establishment_latency` / runtime evidence blockers, so this is not Apollo junction-driving success. | Use the accepted comparison surface to guide Apollo runtime/projection/control-message evidence follow-up; do not claim junction capability. |
| lead_hard_brake | P1 hard-brake following | DONE | `configs/scenarios/baguang/lead_hard_brake_70_to_0_20m.yaml`; `runs/phase1_apollo_sidecar_lead_hard_brake_overlay_online_20260620_004446/analysis/phase1_status/phase1_status.json`; `runs/phase1_apollo_sidecar_lead_hard_brake_overlay_online_20260620_004446/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_builtin_baguang_lead_hard_brake_safety_surface_20260620_004737/analysis/phase1_status/phase1_status.json`; `runs/phase1_builtin_baguang_lead_hard_brake_safety_surface_20260620_004737/analysis/artifact_completeness/artifact_completeness_report.json`; `runs/phase1_acceptance_lead_hard_brake_20260620_155924/comparison/comparison_summary.json`; `runs/phase1_acceptance_lead_hard_brake_20260620_155924/acceptance/phase1_acceptance_report.json`; `tests/test_fixed_scene_compiler.py::test_baguang_lead_hard_brake_profile_matches_70_to_stop`; `tests/test_scenario_comparison.py`; `tests/test_phase1_acceptance.py` | The accepted bundle binds exact ApolloBackend and PlanningControlBackend runs for the Baguang hard-brake target case, resolved `lead_vehicle` target evidence, `v_t_gap`, Phase 1 artifact completeness, and no-blocking-assist gates. Both participating runs remain evaluable `lane_invasion` failures; Apollo link-health reports module-consumption/runtime blockers. This is accepted Phase 1 comparison-surface evidence only, not Apollo hard-brake or natural-driving capability evidence. | Use this accepted bundle as target-following blocker evidence; do not treat it as a behavior pass. |
...
- Apollo scaffold comparisons now exist for all current P0/P1 catalog rows, and
  eight real Apollo-vs-builtin Phase 1 comparisons now exist, including:
...
- Current catalog completion is governed by accepted bundles, not by the old
  cross-run readiness snapshot. After the cut-in target-evaluability fix,
  current P0/P1 catalog coverage is `8 DONE / 0 PARTIAL`, with all DONE rows
  backed by accepted bundles.
