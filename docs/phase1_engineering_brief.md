...
- `lane_keep_straight`, `follow_stop_static`, `lead_decel_accel`,
  `lane_keep_curve`, `junction_turn_no_signal`, and `lead_hard_brake` now have
  accepted `phase1_acceptance_report.json` bundles binding one exact
  ApolloBackend run, one exact PlanningControlBackend run, and one exact
  ScenarioComparison with all Phase 1 validity gates true.
- The next phase-progress milestone is extending this atomic accepted-bundle
  pattern to the two remaining P0/P1 target-transition scenarios,
  `cut_in_simple` and `cut_out_simple`, not returning to cross-run best-of
  accounting.
- Existing comparable failures remain useful blocker evidence. They must not be
  rewritten as Apollo natural-driving success or as Phase 1 completion.
...
| cut_in_simple | P0 target enters ego lane | PARTIAL | `configs/scenario_templates/cut_in.yaml`; `configs/scenarios/baguang/cut_in_35kph_left_to_right_10m.yaml`; `configs/scenarios/town01/cut_in_097.yaml`; `runs/phase1_apollo_sidecar_cut_in_overlay_online_20260620_005245/analysis/phase1_status/phase1_status.json`; `runs/phase1_builtin_baguang_cut_in_safety_surface_20260620_005800/analysis/phase1_status/phase1_status.json`; `runs/phase1_comparisons/baguang_cut_in_apollo_overlay_vs_builtin_safety_surface_20260620_005828/comparison_summary.json`; `tests/test_fixed_scene_player.py::test_player_triggers_baguang_cut_in_on_longitudinal_gap`; `tests/test_phase1_status_classifier.py::test_early_lane_invasion_before_phase_activation_is_evaluable_failure` | Cut-in has same-case ApolloBackend-vs-PlanningControlBackend comparison evidence with matching safety-event surfaces, but no accepted bundle. The representative runs are evaluable `failed/lane_invasion`, and the Apollo run reaches lane invasion before the target activation phase, so target interaction was not accepted. | Use this as a comparable failure for blocker triage; do not claim target-transition success until a run completes the activation phase and remains evaluable. |
| lane_keep_straight | P0 lateral baseline | DONE | `configs/scenarios/town01/lane_keep_097.yaml`; `runs/phase1_online_builtin_lane_keep_safety_surface_20260619_163934/`; `runs/apollo_l1_accel_filter_tuned_20260616_115254/.../analysis/phase1_status/phase1_status.json`; `runs/phase1_acceptance_lane_keep_straight_20260620_153947/comparison/comparison_summary.json`; `runs/phase1_acceptance_lane_keep_straight_20260620_153947/acceptance/phase1_acceptance_report.json`; `tests/test_phase1_scenario_catalog.py`; `tests/test_scenario_comparison.py`; `tests/test_phase1_acceptance.py`; `tests/test_run_artifact_completeness.py` | The accepted bundle binds the exact ApolloBackend run, PlanningControlBackend run, ScenarioComparison, Phase 1 artifact-completeness reports, route-only target metric `not_applicable`, and no-blocking-assist gates. Apollo's participating run is still an evaluable `lane_invasion` failure, so this is accepted Phase 1 comparison-surface evidence, not Apollo natural-driving pass evidence. Any natural-driving claim would still require `natural_driving_report.json` plus the no-interference artifacts. | Keep lane-keep comparison evidence current and focus follow-up on reducing the representative Apollo blocker while extending accepted bundles to the remaining P0/P1 cases. |
...
| cut_out_simple | P1 target exits ego lane | PARTIAL | `configs/scenario_templates/cut_out.yaml`; `configs/scenarios/town01/cut_out_097.yaml`; `configs/scenarios/baguang/cut_out_35kph_right_to_left_25m.yaml`; `runs/phase1_apollo_sidecar_cut_out_overlay_online_20260620_010728/analysis/phase1_status/phase1_status.json`; `runs/phase1_builtin_baguang_cut_out_safety_surface_20260620_010655/analysis/phase1_status/phase1_status.json`; `runs/phase1_comparisons/baguang_cut_out_apollo_overlay_vs_builtin_safety_surface_20260620_010954/comparison_summary.json`; `carla_testbed/scenario_player/templates/cut_out.py`; `tests/test_fixed_scene_compiler.py::test_baguang_cut_out_compiles_lane_change_phase`; `tests/test_apollo_fixed_scene_launch_plan.py::test_apollo_baguang_cut_out_launch_plan_uses_sidecar_runtime_command` | Cut-out has a Baguang fixed-scene ScenarioCase plus same-case ApolloBackend-vs-PlanningControlBackend comparison evidence, but no accepted bundle. Both representative runs are evaluable `failed/lane_invasion`. The Baguang cut-out variant uses a fixed 35s diagnostic duration so lane-change playback can be validated without relying on road-end progression. | Keep Town01 cut-out as future map-specific coverage; use the Baguang comparable failure to triage early lane invasion and Apollo module-consumption blockers. |
...
- The previous generated catalog reading of `8` DONE scenarios was too
  ...
  before target activation. The new Baguang cut-out Apollo sidecar run
  `runs/phase1_apollo_sidecar_cut_out_overlay_online_20260620_010728` and
  builtin run `runs/phase1_builtin_baguang_cut_out_safety_surface_20260620_010655`
  are evaluable `failed/lane_invasion` runs. Their comparisons are comparable,
  which proves evaluability and matched evidence surfaces, not behavior
  success.
...
- `cut_in_simple` now has Baguang Apollo sidecar evidence, refreshed
  `carla_builtin` evidence, and a comparable ApolloBackend-vs-PlanningControlBackend
  report. Its representative runs are evaluable failures, and the Apollo run
  reaches lane invasion before target activation, so this is not target
  interaction success.
- `cut_out_simple` now has a Baguang fixed-scene ScenarioCase,
  ...
  Apollo sidecar evidence, online `carla_builtin` evidence, and a comparable
  ApolloBackend-vs-PlanningControlBackend report. Its representative runs are
  evaluable failures, so this is scenario-platform readiness, not a cut-out
  behavior pass.
- Current catalog completion is governed by accepted bundles, not by the old
  8/8 readiness snapshot.
