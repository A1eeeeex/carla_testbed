...
- Apollo scaffold comparisons now exist for all current P0/P1 catalog rows, and
  six real Apollo-vs-builtin Phase 1 comparisons now exist, including:
...
- The generated catalog currently reports `6` DONE scenarios, `2` PARTIAL, and
  `0` NOT_YET. The DONE rows are `follow_stop_static`,
  `lead_decel_accel`, `lane_keep_straight`, `lane_keep_curve`,
  `junction_turn_no_signal`, and `lead_hard_brake`. DONE means Phase 1
  scenario-platform evidence is present; it does not mean Apollo natural
  driving success. The new hard-brake Apollo sidecar run
  `runs/phase1_apollo_sidecar_lead_hard_brake_overlay_online_20260620_004446`
  is evaluable but failed with `lane_invasion`. The refreshed builtin
  hard-brake run
  `runs/phase1_builtin_baguang_lead_hard_brake_safety_surface_20260620_004737`
  is also evaluable but failed with `lane_invasion`. Their comparison is
  `comparable`, which proves evaluability and matched evidence surfaces, not
  behavior success. Remaining PARTIAL rows are `cut_in_simple` and
  `cut_out_simple`, both still missing evaluable ApolloBackend fixed-scene
  target-transition evidence.
...
- P1 `lead_hard_brake` now has a Baguang ScenarioCase YAML, compiler test,
  ...
  capability pass.
- The current catalog reports `6` DONE scenarios, `2` PARTIAL, and `0`
  `NOT_YET`. The remaining PARTIAL rows are cut-in/cut-out target-transition
  cases.
...
- Generic `ScenarioCase x Backend` comparison artifacts exist. `lane_keep_097`,
  `follow_stop_097`, lead-decel, hard-brake, curve, and junction now have real
  Apollo-vs-builtin comparable reports.
  ...
  The remaining cut-in/cut-out fixed-scene target-transition scenarios are
  still only partially evaluable because ApolloBackend runs are scaffold,
  readiness-only, invalid, or missing.
...
- Evaluable ApolloBackend fixed-scene target-transition evidence for cut-in and
  cut-out.
