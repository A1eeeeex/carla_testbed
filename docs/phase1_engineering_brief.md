...
- `lane_keep_straight` now has an accepted
  `phase1_acceptance_report.json` binding one exact ApolloBackend run, one
  exact PlanningControlBackend run, and one exact ScenarioComparison with all
  Phase 1 validity gates true.
