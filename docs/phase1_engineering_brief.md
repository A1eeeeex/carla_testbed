...
- Full CI-friendly pytest passed locally after the route-only curve Phase 1
  normalization update: `1860 passed in 65.21s`.
...
- Apollo scaffold comparisons now exist for all current P0/P1 catalog rows, and
  three real Apollo-vs-builtin Phase 1 comparisons now exist:
...
- The generated catalog currently reports `3` DONE scenarios, `5` PARTIAL, and
  `0` NOT_YET. `lane_keep_straight`, `lane_keep_curve`, and
  `follow_stop_static` are DONE as Phase 1 scenario-platform evidence, not as
  Apollo natural-driving success.
