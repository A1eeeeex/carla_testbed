...
- `cut_in_simple` now has Baguang Apollo sidecar evidence, refreshed
  ...
  report, and a generated `phase1_acceptance_report.json` bundle. The
  ...
  for that sample. This is improved comparison-surface evidence, not Apollo
  cut-in behavior success, and the row remains `PARTIAL` until the accepted
  bundle is self-contained and recomputable.
