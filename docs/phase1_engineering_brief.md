...
- `lane_keep_straight`, `follow_stop_static`, `lead_decel_accel`,
  `lane_keep_curve`, `cut_out_simple`, `junction_turn_no_signal`, and
  `lead_hard_brake` now have accepted `phase1_acceptance_report.json` bundles
  binding one exact ApolloBackend run, one exact PlanningControlBackend run,
  and one exact ScenarioComparison with all Phase 1 validity gates true.
- The next phase-progress milestone is extending this atomic accepted-bundle
  pattern to the remaining P0 target-transition scenario, `cut_in_simple`, not
  returning to cross-run best-of accounting.
...
- Current catalog completion is governed by accepted bundles, not by the old
  8/8 readiness snapshot; after the cut-out analyzer fix, the remaining
  non-accepted scenario is `cut_in_simple`.
