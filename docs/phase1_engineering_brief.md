...
- The next phase-progress milestone is no longer catalog coverage. It is
  ...
  `prediction_evidence:closed_loop` link-health blocker.
...
| cut_in_simple | P0 target enters ego lane | DONE | `configs/scenario_templates/cut_in.yaml`; `configs/scenarios/baguang/cut_in_35kph_left_to_right_10m.yaml`; `configs/scenarios/town01/cut_in_097.yaml`; `runs/phase1_apollo_sidecar_cut_in_target_evaluable_online_20260620_165920/analysis/phase1_status/phase1_status.json`; `runs/phase1_builtin_baguang_cut_in_target_evaluable_20260620_170548/analysis/phase1_status/phase1_status.json`; `runs/phase1_comparisons/baguang_cut_in_target_evaluable_refreshed_20260620_170727/comparison_summary.json`; `runs/phase1_acceptance_cut_in_simple_20260620_170727/acceptance/phase1_acceptance_report.json`; `tests/test_fixed_scene_player.py::test_baguang_cut_in_starts_at_configured_activation_gap`; `tests/test_scenario_actor_contract.py::test_lane_change_contract_uses_actor_world_motion_when_ego_relative_lateral_drifts`; `tests/test_phase1_status_classifier.py::test_duration_policy_not_reached_after_target_interaction_is_evaluable_failure` | The accepted bundle binds exact ApolloBackend and PlanningControlBackend runs for the Baguang cut-in target-transition case. The scenario now starts inside the target activation window so both backends exercise `cut_in_lane_change`; actor-world motion is used for no-teleport evidence when ego-relative lateral is contaminated by ego lane departure. Both participating runs remain evaluable `lane_invasion` failures, so this is accepted Phase 1 comparison-surface evidence only, not Apollo cut-in behavior success. | Use this accepted bundle as target-transition blocker evidence; next work should reduce Apollo/Baguang lane-event and `prediction_evidence:closed_loop` blockers without weakening no-interference or natural-driving claim gates. |
...
- `cut_in_simple` now has Baguang Apollo sidecar evidence, refreshed
  ...
  This is accepted Phase 1 comparison-surface evidence, not Apollo cut-in
  behavior success.
