...
- The low-capture online run
  ...
  link-health primary blocker shifts to
  `route_establishment:scenario_route_lane_ids` because the fixed-scene
  compatibility run lacks claim-grade scenario route lane ids/length and HDMap
  projection rows. The next highest-value work is therefore route/projection
  evidence for the Baguang fixed-scene case, not bridge smoothing or PID tuning.
