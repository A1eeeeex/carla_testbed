...
- `cut_in_simple` is the clearest blocker: the latest refreshed Apollo runs now
  ...
  still fail with `lane_invasion`. The newest Apollo sidecar sample
  `runs/phase1_apollo_sidecar_cut_in_prediction_log_online_20260620_175400`
  captures `artifacts/apollo_prediction.INFO` and shows internal Prediction
  evaluator activity, but `/apollo/prediction` native output is still not
  observed; Apollo link health therefore remains blocked by
